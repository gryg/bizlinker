'use client'
import CreateCampaignPage from '@/components/forms/campaign-page'
import CustomModal from '@/components/global/custom-modal'
import { AlertDialog } from '@/components/ui/alert-dialog'
import { Button } from '@/components/ui/button'
import { ScrollArea } from '@/components/ui/scroll-area'
import { toast } from '@/components/ui/use-toast'
import { upsertCampaignPageDetails } from '@/lib/queries'
import { CampaignsForSubSidiary } from '@/lib/types'
...
import CampaignPagePlaceholder from '@/components/icons/campaign-page-placeholder'
...
type Props = {
  campaign: CampaignsForSubSidiary
  subsidiaryId: string
  pages: CampaignPage[]
  campaignId: string
}

const CampaignSteps = ({ campaign, campaignId, pages, subsidiaryId }: Props) => {
  const [clickedPage, setClickedPage] = useState<CampaignPage | undefined>(
    pages[0]
  )
    ...
    newPageOrder.forEach(async (page, index) => {
      try {
        ...
        toast({
          variant: 'destructive',
          title: 'Failed',
          description: 'Unable to save page order',
        })
        return
      }
    })
        ...
        <aside className="flex-[0.3] bg-background p-6  flex flex-col justify-between ">
          ...
          <Button
            ...
            onClick={() => {
              setOpen(
                <CustomModal
                  title=" Create or Update a Campaign Page"
                  subheading="Campaign Pages allow you to create step by step processes for customers to follow"
                >
                  <CreateCampaignPage
              ...
              <CardHeader>
                ...
                <CardDescription className="flex flex-col gap-4">
                  <div className="border-2 rounded-lg sm:w-80 w-full  overflow-clip">
                    <Link
                      href={`/subsidiary/${subsidiaryId}/campaigns/${campaignId}/editor/${clickedPage?.id}`}
                      className="relative group"
                    >
                      <div className="cursor-pointer group-hover:opacity-30 w-full">
                        <CampaignPagePlaceholder />
                      </div>
                      <LucideEdit
                        size={50}
                    ...
                    <Link
                      target="_blank"
                      href={`${process.env.NEXT_PUBLIC_SCHEME}${campaign.subDomainName}.${process.env.NEXT_PUBLIC_DOMAIN}/${clickedPage?.pathName}`}
                    ...
                    >
                      ...
                      <div className="w-64 overflow-hidden overflow-ellipsis ">
                        ...
                        {campaign.subDomainName}.{process.env.NEXT_PUBLIC_DOMAIN}/
