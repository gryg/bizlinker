...
const page = async ({ params }: Props) => {
  ...
  const firmSubscription = await db.firm.findUnique({
    where: {
      id: params.firmId,
    },
    select: {
  ...
  const currentPlanDetails = pricingCards.find(
    (c) => c.priceId === firmSubscription?.Subscription?.priceId
  )

  const charges = await stripe.charges.list({
    limit: 50,
    customer: firmSubscription?.customerId,
  })
  ...
  return (
    <>
      <SubscriptionHelper
        prices={prices.data}
        customerId={firmSubscription?.customerId || ''}
        planExists={firmSubscription?.Subscription?.active === true}
      />
      ...
      <div className="flex flex-col lg:!flex-row justify-between gap-8">
        <PricingCard
          ...
          customerId={firmSubscription?.customerId || ''}
          amt={
            firmSubscription?.Subscription?.active === true
          ...
          further question contact 0001xxxx@protonmail.com"
