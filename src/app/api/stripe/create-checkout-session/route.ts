...
export async function POST(req: Request) {
  const {
    ...
    subSidiaryConnectAccId: string
    prices: { recurring: boolean; productId: string }[]
    subsidiaryId: string
  } = await req.json()

  const origin = req.headers.get('origin')
  if (!subSidiaryConnectAccId || !prices.length)
  ...
  if (
    ...
    !process.env.NEXT_PUBLIC_PLATFORM_SUBSIDIARY_PERCENT
  ) {
    console.log('VALUES ARE NOT SET IN .ENV FILE')
  ...
  try {
    const session = await stripe.checkout.sessions.create(
      ...
      { stripeAccount: subSidiaryConnectAccId }
    )
