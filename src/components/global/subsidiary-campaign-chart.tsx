...
const SubsidiaryCampaignChart = ({ data }: Props) => {
  return (
    <div className="h-fit flex transition-all items-start">
      <DonutChart
        className="h-40 w-40"
        data={data}
        category="totalCampaignVisits"
        index="name"
...
export default SubsidiaryCampaignChart
...
const customTooltip = ({
  ...
  return (
    <div className="ml-[100px] dark:text-white text-black w-fit dark:bg-muted/60 backdrop-blur-md bg-background/60 !rounded-lg p-2 shadow-2xl">
      ...
      {categoryPayload.payload.CampaignPages?.map((page: any) => (
        <div
          key={page.id}
