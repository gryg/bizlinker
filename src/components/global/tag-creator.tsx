...
const TagCreator = ({ getSelectedTags, subSidiaryId, defaultTags }: Props) => {
  ...
  useEffect(() => {
    if (subSidiaryId) {
      const fetchData = async () => {
        const response = await fetchTagsForSubsidiary(subSidiaryId)
  ...
  const handleAddTag = async () => {
    ...
    setSelectedColor('')
    try {
      const response = await upsertTagDetails(subSidiaryId, tagData)
      toast({
  ...
  const handleDeleteTag = async (tagId: string) => {
    ...
    try {
      ...
      toast({
        title: 'Deleted tag',
        description: 'The tag is deleted from your subsidiary.',
      })
