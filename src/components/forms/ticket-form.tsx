...
const TicketForm = ({ getNewTicket, laneId, subsidiaryId }: Props) => {
  ...
  useEffect(() => {
    if (subsidiaryId) {
      const fetchData = async () => {
        const response = await fetchSubSidiaryTeamMembers(subsidiaryId)
  ...
  const onSubmit = async (values: z.infer<typeof TicketFormSchema>) => {
    if (!laneId) return
    try {
      const response = await upsertTicketDetails(
        {
          ...values,
          laneId,
